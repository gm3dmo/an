...
def __belongsTo(value, rangeStart, rangeEnd):
...
#Voluntary Aid Detachments W/500001 - W/1000000
    ...
    for unit in army_number_index:
        original_unit = __belongsTo(army_number, army_number_index[unit][0], army_number_index[unit][1]  )
...
def create_app(config=None):
    ...
    def foo_url_arg(someId):
        logger.info("/army-number/%s", someId)
        ou = original_unit(int(someId))
        return jsonify({"original-unit-joined": ou, "army-number": someId})
